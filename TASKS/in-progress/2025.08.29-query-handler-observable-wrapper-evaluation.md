...
## 📐 Simplified Architecture Design (No QueryMetadata Needed!)
...
#### Step 2: Create PlainSnapshotWrapper
- **New file**: `example/shared/shell/snapshot/plain_wrapper.go`
...
- **Purpose**: Pure snapshot logic only, no observability
- **Strategy**: Coexists with `GenericSnapshotWrapper` during migration
- **Migration plan**: After Phase 3, delete old wrapper and rename Plain → Generic
...
- **Update `query_handler_test.go`**:
  ...
  - Composition: QueryHandler → PlainSnapshotWrapper → ObservableWrapper

- **Update `query_handler_snapshot_test.go`**:
  - Remove t.Skip() for booksincirculation tests ONLY (3 tests)
  - Use PlainSnapshotWrapper instead of GenericSnapshotWrapper
...
- **Update simulation2/handlers.go** (booksincirculation only):
  ...
  snapshotHandler, err := snapshot.NewPlainSnapshotWrapper(
      coreHandler,
...
### ⚠️ IMPORTANT: Phase 2 Scope Limits
...
- ✅ ObservableWrapper created and tested with mocks
- ✅ PlainSnapshotWrapper created (coexists with GenericSnapshotWrapper)
- ✅ BooksInCirculation handler cleaned (zero observability code)
- ✅ BooksInCirculation tests updated with new composition
- ✅ 3 BooksInCirculation snapshot tests un-skipped and passing
- ✅ Simulation still works with new composition for booksincirculation
- ✅ Other 6 handlers still work unchanged
...
4. **Update simulation bootstrapping** in `simulation2/handlers.go`:
   ...
   - To: `NewCoreQueryHandler(eventStore)` → `SnapshotWrapper` → `ObservableWrapper`
   - Correct composition order: **Core → Snapshot (optional) → Observable**
