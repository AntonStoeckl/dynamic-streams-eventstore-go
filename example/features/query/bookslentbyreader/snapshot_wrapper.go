...
func (h *SnapshotAwareQueryHandler) Handle(ctx context.Context, query Query) (BooksCurrentlyLent, error) {
	...
	baseFilter := BuildEventFilter(query.ReaderID)

	// Snapshot Load phase
	snapshot, err := h.executeSnapshotLoad(ctx, query, baseFilter)
	if err != nil {
	...
	result := h.executeIncrementalProjection(ctx, query, incrementalEvents, baseProjection, finalSequence)

	// Save the updated snapshot with incremental changes
	h.saveUpdatedSnapshot(ctx, query, baseFilter, finalSequence, result)
...
func (h *SnapshotAwareQueryHandler) executeSnapshotDeserialization(
	...
	var baseProjection BooksCurrentlyLent
...
func (h *SnapshotAwareQueryHandler) executeIncrementalProjection(
	...
	baseProjection BooksCurrentlyLent,
...
func (h *SnapshotAwareQueryHandler) recordFallbackAndExecute(
	...
	if fallbackReason == shell.SnapshotReasonMiss {
		...
		h.saveUpdatedSnapshot(ctx, query, baseFilter, result.SequenceNumber, result)
	}
