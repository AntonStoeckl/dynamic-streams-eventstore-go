...
func Test_SnapshotAwareQueryHandler_Handle_SnapshotCreationAndHitWithNoNewEvents(t *testing.T) {
	...
	savedSnapshot, err := wrapper.GetEventStore().LoadSnapshot(ctx, "BooksInCirculation", filter)
...
func Test_SnapshotAwareQueryHandler_Handle_SnapshotHitWithNewEvents(t *testing.T) {
	...
	updatedSnapshot, err := wrapper.GetEventStore().LoadSnapshot(ctx, "BooksInCirculation", filter)
...
func setupSnapshotTestWithMetrics(t *testing.T) (context.Context, *booksincirculation.SnapshotAwareQueryHandler, *MetricsCollectorSpy, Wrapper) {
	t.Helper()
	...
	snapshotHandler, err := booksincirculation.NewSnapshotAwareQueryHandler(baseHandler)
