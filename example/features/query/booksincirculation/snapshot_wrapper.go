...
func (h *SnapshotAwareQueryHandler) Handle(ctx context.Context) (BooksInCirculation, error) {
...
) (BooksInCirculation, error) {

	deserializationStart := time.Now()
	var baseProjection BooksInCirculation
...
func (h *SnapshotAwareQueryHandler) executeIncrementalProjection(
	...
	baseProjection BooksInCirculation,
	maxSequence eventstore.MaxSequenceNumberUint,
) BooksInCirculation {

	incrementalProjectionStart := time.Now()
	result := ProjectBooksInCirculation(incrementalEvents, maxSequence, baseProjection)
