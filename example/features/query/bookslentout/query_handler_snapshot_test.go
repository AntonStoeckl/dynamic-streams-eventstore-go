...
func Test_SnapshotAwareQueryHandler_Handle_SnapshotCreationAndHitWithNoNewEvents(t *testing.T) {
	...
	savedSnapshot, err := wrapper.GetEventStore().LoadSnapshot(ctx, snapshotHandler.BuildSnapshotType(bookslentout.BuildQuery()), filter)
...
func Test_SnapshotAwareQueryHandler_Handle_SnapshotHitWithNewEvents(t *testing.T) {
	...
	updatedSnapshot, err := wrapper.GetEventStore().LoadSnapshot(ctx, snapshotHandler.BuildSnapshotType(bookslentout.BuildQuery()), filter)
	...
	var updatedProjection bookslentout.BooksLentOut
	...
	assert.Equal(t, uint(6), updatedProjection.SequenceNumber, "Updated snapshot projection should have sequence=6")
}

// Helper function to set up the test environment with metrics spy.
func setupSnapshotTestWithMetrics(t *testing.T) (
	context.Context,
	*snapshot.GenericSnapshotWrapper[bookslentout.Query, bookslentout.BooksLentOut],
	*MetricsCollectorSpy,
	Wrapper,
) {
	...
	baseHandler, err := bookslentout.NewQueryHandler(
		wrapper.GetEventStore(),
		bookslentout.WithMetrics(metricsCollector),
	)
