...
func Test_SnapshotAwareQueryHandler_Handle_SnapshotCreationAndHitWithNoNewEvents(t *testing.T) {
	...
	savedSnapshot, err := wrapper.GetEventStore().LoadSnapshot(ctx, snapshotHandler.BuildSnapshotType(canceledreaders.BuildQuery()), filter)
...
func Test_SnapshotAwareQueryHandler_Handle_SnapshotHitWithNewEvents(t *testing.T) {
	...
	// Second query: Should hit snapshot and process incremental events (new canceled reader)
	...
	filter = canceledreaders.BuildEventFilter()
	updatedSnapshot, err := wrapper.GetEventStore().LoadSnapshot(ctx, snapshotHandler.BuildSnapshotType(canceledreaders.BuildQuery()), filter)
...
func setupSnapshotTestWithMetrics(t *testing.T) (
	context.Context,
	*snapshot.GenericSnapshotWrapper[canceledreaders.Query, canceledreaders.CanceledReaders],
	*MetricsCollectorSpy,
	Wrapper,
) {
	...
	baseHandler, err := canceledreaders.NewQueryHandler(
		wrapper.GetEventStore(),
		canceledreaders.WithMetrics(metricsCollector),
	)
