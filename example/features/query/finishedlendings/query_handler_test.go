...
func Test_QueryHandler_Handle_IncludesFinishedLendingsSortedByReturnedAt(t *testing.T) {
	...
	err := handlers.lendBook.Handle(ctx, lendCmd1)
	...
	err = handlers.returnBook.Handle(ctx, returnCmd1)
	...
	err = handlers.lendBook.Handle(ctx, lendCmd2)
	...
	err = handlers.returnBook.Handle(ctx, returnCmd2)
...
func Test_QueryHandler_Handle_HandlesComplexLendingScenarios(t *testing.T) {
	...
	err = handlers.lendBook.Handle(ctx, lendCmd3)
	...
	err = handlers.returnBook.Handle(ctx, returnCmd3)
...
func createAllHandlers(t *testing.T, wrapper Wrapper) testHandlers {
	...
	assert.NoError(t, err, "Should create LendBookCopyToReader handler")

	returnBookHandler, err := returnbookcopyfromreader.NewCommandHandler(wrapper.GetEventStore())
	assert.NoError(t, err, "Should create ReturnBookCopyFromReader handler")
